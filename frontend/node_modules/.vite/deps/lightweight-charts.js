...
function normalizeAlphaComponent(component) {
	return !(component <= 0) && !(component > 0) ? 0 : component < 0 ? 0 : component > 1 ? 1 : Math.round(component * 1e4) / 1e4;
}
/**
* @example
* #fb0
	...
	_internal_draw(target, rendererOptions, textWidthCache, align) {
		...
		const geometry = target.useBitmapCoordinateSpace((scope) => {
			...
			const drawLabelBody = (labelBackgroundColor, labelBorderColor) => {
				if (geom._internal_alignRight) drawRoundRectWithInnerBorder(ctx, gb._internal_xOutside, gb._internal_yTop, gb._internal_totalWidth, gb._internal_totalHeight, labelBackgroundColor, gb._internal_horzBorder, [
					gb._internal_radius,
					0,
					0,
					gb._internal_radius
				], labelBorderColor);
				else drawRoundRectWithInnerBorder(ctx, gb._internal_xInside, gb._internal_yTop, gb._internal_totalWidth, gb._internal_totalHeight, labelBackgroundColor, gb._internal_horzBorder, [
					0,
					gb._internal_radius,
					gb._internal_radius,
					0
...
function drawItem(item, ctx) {
	...
	if (item._internal_text !== void 0) drawText(ctx, item._internal_text._internal_content, item._internal_text._internal_x, item._internal_text._internal_y);
	drawShape(item, ctx);
}
...
var PriceScale = class {
	...
	_internal_applyOptions(options) {
		...
		if (options.scaleMargins !== void 0) {
			...
			if (bottom < 0 || bottom > 1 || top + bottom > 1) throw new Error(`Invalid bottom margin - expect value between 0 and 1, given=${bottom}`);
...
var PriceAxisWidget = class {
	...
	_private__fixLabelOverlap(views, rendererOptions, center) {
		...
		for (let i = 1; i < top.length; i++) {
			...
			if (coordinate > prevFixedCoordinate - height) view._internal_setFixedCoordinate(prevFixedCoordinate - height);
		}
